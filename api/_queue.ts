...
export async function processQueue() {
  ...
  try {
    ...
    const { data: dueEmails, error: fetchError } = await supabase
      ...
      .limit(3); // Reduced to 3 for even more safety on Hobby plan
    ...
    for (const dueEmail of dueEmails) {
      console.log(`Processing email for: ${dueEmail.email}`);
      ...
      if (claimError || !claimed) {
          console.log(`Could not claim email ${dueEmail.id}, skipping...`);
          continue;
      }
      ...
      try {
        ...
        sentCount++;
        console.log(`Successfully sent email to ${dueEmail.email}`);
